...
public class Kaiwa extends JFrame implements Runnable {
	...
	private void meke_help_frame() {
		...
		System.out.println(
				"ttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttrect.width==" + rect.width);
	...
	private Prop get_property(Prop prop) {
		...
		try {
			...
			for (int j = 0; j < 10; j++) {
				...
				category_img_from_property[j] = prop.getPict()
						.getProperty("seting.category_img_name" + String.valueOf(j));// カテゴリ画像名文字列

				category_visible_boolean_string[j] = new String();
				category_visible_boolean_string[j] = prop.getPict()
						.getProperty("seting.mokujiBool" + String.valueOf(j));// カテゴリ
																				// 表示true
																				// or
																				// false
	...
	private JTabbedPane make_pict_tab() {
		...
		for (int i = 0; i < tabPanel_0_9.length; i++) {
			...
			if (category_visible_boolean[i].equals(true)) {
				...
				if (0 == i) {
					ZoomTabImg = new ZoomTab(
							new ImageIcon("./resource/img/tab1/" + category_img_from_property[i] + ".jpg"), 0, 0, 50,
							50);
				} else {
					ZoomTabImg = new ZoomTab(
							new ImageIcon("./resource/img/tab1/" + category_img_from_property[i] + ".jpg"), 0, 0, 50,
				...
				for (int j = 0; j < category_tate_kakeru_yoko_num; j++) {
					...
					if (i == 9) {// アプリ分野
						...
						if (category_files_num <= j) {
							...
							ZoomImg = new Zoom(
									new ImageIcon("./resource/img/" + category_directory_9[i] + "/" + "non.jpg"), 0, 0,
									buttons_0_9[i][j].getHeight(), buttons_0_9[i][j].getHeight());
							...
							ZoomImg = new Zoom(
									new ImageIcon("./resource/img/" + category_directory_9[i] + "/"
											+ bunyaFiles_0_9[i][j].getName()),
									0, 0, buttons_0_9[i][j].getHeight(), buttons_0_9[i][j].getHeight());
	...
	public void mokuji(JPanel[] category_botton_panel) {
		...
		for (int i = 1; i < 10; i++) {
			...
			if (category_visible_boolean[i].equals(true)) {
				...
				Zoom zoomImage = new Zoom(
						new ImageIcon("./resource/img/tab1/" + category_img_from_property[i] + ".jpg"), 0, 0,
						category_button_9[i].getHeight(), category_button_9[i].getHeight());
		...
		while (true) {
			...
			while (yelllow_loop_select_flag) {
				...
				panels_0_9[loop_index][yoko_Num * yoko_Inc2 + tate_Inc]
						.setBorder(new LineBorder(Color.white, 0, false));
				...
				if (list_select_flag) {
					...
					if (tabbedPane.getSelectedIndex() == trueCount - 1) {
						...
						new l(this,
								"buttons_0_9==" + buttons_0_9[loop_index][yoko_Inc2 * yoko_Num + tate_Inc].getText());
						...
						if (buttons_0_9[loop_index][yoko_Inc2 * yoko_Num + tate_Inc].getText().equals("tab10")) {
							...
							File f = new File("./resource/img/tab10/"
									+ buttons_0_9[loop_index][yoko_Inc2 * yoko_Num + tate_Inc].getText() + ".jar");
								...
								if (gyouIs.equals("②絵記号を拡大表示")) {
									...
									Zoom zoom = new Zoom(
											new ImageIcon("./resource/img/" + category_directory_9[loop_index] + "/"
													+ buttons_0_9[loop_index][yoko_Inc2 * yoko_Num + tate_Inc].getText()
													+ ".jpg"),
											0, 0, 70, 60);

									new l(this,
											"./resource/img/" + category_directory_9[loop_index] + "/"
													+ buttons_0_9[loop_index][yoko_Inc2 * yoko_Num + tate_Inc].getText()
													+ ".jpg");
			...
			if (tabbedPane.getSelectedIndex() == 0) {// タブインデックスが変化したら目次ループへ
				 ...
				 * LineBorder(Color.black, 0, false));//NG
				 */
				for (int i = 1; i < 10; i++) {
					if (category_visible_boolean[i].equals(true)) {
	...
	void mokujiLoop() {
		...
		try {
			...
			if (category_visible_boolean[loop_index].equals(true)) {
				...
				new l(this, "mokujiLoop>mokujiName_10[mokujiBIndexI] == "
						+ StringFormat.stringFormat(category_from_property[loop_index], 10) + "：" + loop_index);
				Thread.sleep(speed);
			}
		...
		public void mousePressed(MouseEvent e) {
			...
			for (int i = 1; i < tabPanel_0_9.length; i++) {
				...
				if (category_visible_boolean[i].equals(true)) {
					...
					for (int j = 0; j < buttons_0_9[i].length; j++) {
						// 押されたボタンである
						if (i != 9 && e.getSource() == buttons_0_9[i][j]
							...
							// アプリ分野
						} else if ((i == 9 && e.getSource() == buttons_0_9[9][j])
