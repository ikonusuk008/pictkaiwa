...
public class List extends JFrame implements Runnable,ActionListener {
    ...
    public List(String ekigoName, String folderName) throws HeadlessException, SQLException {
        ...
        for (int i = table_row_start_num; i < table_row_finish_num; i++) {
            ...
            Zoom zoom = new Zoom(new ImageIcon("./resource/img/" + folderName + "_詳細/" + listURL[i]), 0, 0,
    ...
    private void make_new_pict_table(String ekigoName) {
        ...
        if (isFile) {// true:すでにファイルがある
            ...
            new Lg(this, "isFile>true: テーブル有り：作成しない");
             ...
             * false:ファイルがないので作成する
             */
            new Lg(this, "isFile>false: テーブルなし：作成する");
            new Write(ekigoName);
            ...
            for (int j = table_row_start_num; j < table_row_finish_num; j++) {
                ...
                try {
                    ...
                    if (file_name_list[j] == null) {// nullであれば・をいれる
                        ...
                        stmt.executeUpdate("insert into " + ekigo + "_t" + "(NO," + ekigo + ", url)" + "values("
                ...
                if (selected == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        imgButton[i].add(new Zoom(
                                        new ImageIcon("./resource/img/" + folder + "_詳細/"
                ...
                if (false == textArea[i].getText().equals(" ")) {
                    ...
                    if (textArea[i].getText().equals(cmd)) {
                        ...
                        try {
                            ...
                            /*
                             * レコードを削除する：レコード自体が消えてしまうためNG stmt.executeUpdate("DELETE  FROM " + ekigo + "_t WHERE " +
                             * ekigo + "='" + textArea[i].getText() + "';"); レコードを更新する。
                             */
                            stmt.executeUpdate("update " + ekigo + "_t set " + ekigo + "='" + "" + "' where " + "no"
                                            + "=" + String.valueOf(i) + ";");

                            stmt.executeUpdate("insert into " + ekigo + "_t" + "(NO," + ekigo + ")" + "values("
                                            + String.valueOf(ristNO[i - 1]) + ",'" + " " + "');");// インデックスを元の番号に書き直した-20100429
                            stmt.executeUpdate("update " + ekigo + "_t set " + "url" + "= '" + "default.jpg"
                                            + "' where " + " NO" + "=" + imgButton[i].getText() + ";");
                            /*
                             * executeInsert("DELETE  FROM うで_t WHERE うで='test';"); o("DELETE  FROM " + fileName +
            ...
            if (keyPressReleaseAvailable == true && keyEventCout == 1) {
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    if (KeyEvent.getKeyText(e.getKeyChar()).equals(String.valueOf(i))
                        ...
                        KaiwaMain.keyPressAvailable = false;// 閉じたときに、Kaiwaフレームにキーイベントが行かないようにする。
